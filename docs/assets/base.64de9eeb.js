...
padding-top: 8px;
font-size: 10px;
`);const u=new f.Application({view:document.querySelector(".orb-canvas"),resizeTo:window,backgroundAlpha:0}),h=new S;u.stage.filters=[new y(30,10,!0)];const d=[];for(let s=0;s<10;s++){const t=new C(h.randomColor());u.stage.addChild(t.graphics),d.push(t)}window.matchMedia("(prefers-reduced-motion: reduce)").matches?d.forEach(s=>{s.update(),s.render()}):u.ticker.add(()=>{d.forEach(s=>{s.update(),s.render()})});document.addEventListener("keypress",s=>{s.key==="n"&&(h.setColors(),h.setCustomProperties(),d.forEach(t=>{t.fill=h.randomColor()}))});new x;a("[data-magnetic]").each(function(){new b(this)});window.addEventListener("load",()=>{const s=document.querySelector(".loading"),t=document.querySelector("main");s!=null&&(s.style.opacity="0",setTimeout(()=>{s.style.display="none",t.style.opacity="1"},500))});
